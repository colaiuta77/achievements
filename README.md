...
![독서 업적 전용 탭](docs/achievements-tab.png?v=1.3.0)
...
- 비디오북 첫 시청, 첫 완주와 누적 완주 업적을 제공합니다.
...
본문은 첫걸음, 완독, 독서량, 연속 독서, 탐험, 오디오북과 비디오북 카테고리로 구분합니다. 각 카드는 희귀도, 목표, 현재 진행률과 달성일을 표시합니다.
...
- 제목·요약 카드에 테마 연동 외곽선과 부드러운 그림자 적용.
- 완료 업적에 희귀도별 이중 테두리와 광택 추가.
- 달성하면 이름과 조건이 공개되는 히든 미션 3개 추가. 달성 전에는 힌트만 표시합니다.
- 비디오 업적의 진행 단위를 편으로 표시.
- 히든 미션 추가에 맞춰 캐시 정의 버전을 갱신하고 미달성 히든 미션은 다음 목표 추천에서 제외.
- 수동 파일 교체 시 Python 정의를 다시 읽도록 BookOasis 컨테이너를 재시작하세요.
...
- BookOasis 2.1.4의 `video` DB와 `videos`, `video_progress` 계약 지원.
- 첫 시청, 첫 완주, 누적 완주 5편·10편·25편의 비디오북 업적 5개 추가.
- `category_tab.sessions`를 문자열 `all`로 지정해 모든 보관함 세션에 독서 업적 메뉴 노출.
- 비디오 진행률은 Redis pending overlay 없이 DB 저장값만 집계.
